<template>
  <v-container>
      ...
      <v-layout>
        <v-app-bar color="purple-darken-3" image="">
          <template v-slot:image>
            <v-img
              gradient="to top right, rgba(19,84,122,0.4), rgba(128,208,199,0.3)"
            ></v-img>
          </template>
          ...
          >
            <v-btn value="Played">
              <v-icon :color="color('Played')" size="30"
                >mdi-progress-check</v-icon
              >
            </v-btn>
            <v-btn value="Playing">
              <v-icon :color="color('Playing')" size="30"
                >mdi-progress-star-four-points</v-icon
              >
            </v-btn>
            <v-btn value="ToPlay">
              <v-icon :color="color('ToPlay')" size="30"
              ...
              <v-layout>
                <v-navigation-drawer v-model="drawer" expand-on-hover rail>
                  ...
                  <v-list density="compact" nav>
                    ...
                    />
                    <v-list-item
                      prepend-icon="mdi-sony-playstation"
                      title="Playstation"
                      @click="platform = 'PlayStation'"
                      :active="platform === 'PlayStation'"
                    />
                    <v-list-item
                      prepend-icon="mdi-nintendo-switch"
                      title="Nintendo Switch"
                      @click="platform = 'Nintendo Switch'"
                      :active="platform === 'Nintendo Switch'"
                    />
                    <v-list-item
                      prepend-icon="mdi-microsoft-xbox"
                      title="Xbox"
                      @click="platform = 'Xbox'"
                ...
                <v-main class="ma-1" style="min-height: 350px">
                  <div class="d-flex flex-wrap ga-5">
                    <v-card
                      v-for="g in filteredGames(status)"
                      :key="g.id"
                      max-height="375"
                      max-width="250"
                      variant="tonal"
                    ...
                    >
                      <v-card-title
                        style="
                          ...
                          height: 3.5em;
                        "
                        >{{ g.title }}</v-card-title
                      >
                      <v-card-subtitle
                        >{{ hourOfPlayedTime(g.played_time) }}h
                        {{ minOfPlayedTime(g.played_time) }}m</v-card-subtitle
                      >
                      <v-img
                        cover
                        :aspect-ratio="1"
                        width="250"
                        :src="`/assets/images/games/${g.id}.webp`"
                      ></v-img>
                      <v-card-actions>
                        <v-btn
                          variant="outlined"
                          density="comfortable"
                          :icon="icon(g.status)"
                          :color="color(g.status)"
                          @click="fetchUpdatingGame(g.id)"
                        />
                        <v-btn
                          variant="outlined"
                          density="comfortable"
                          :icon="icon(g.platform)"
                          :color="color(g.platform)"
                        />
                        <v-btn
                          variant="outlined"
                          density="comfortable"
                          icon
                          @click="
                            openUpdateGameRatingDialog(g.id, g.title, g.rating)
                          "
                        >
                          <v-progress-circular
                            :model-value="percentage(g.rating, 10)"
                          >
    ...
    <div class="pa-4 text-center">
      <v-dialog
        v-model="updateGameRatingDialog"
        max-height="600"
        max-width="450"
      >
        <v-form
          ...
          <v-card prepend-icon="mdi-database-edit-outline" title="Edit Rating">
            ...
            <v-card-actions>
              ...
              <v-btn
                ...
                text="Save"
                variant="tonal"
                @click="updateGameRatingDialog = false"
              ></v-btn>
            </v-card-actions>
          </v-card>
        </v-form>
...
const openUpdateGameRatingDialog = (
  id: string,
  title: string,
  rating: number
) => {
